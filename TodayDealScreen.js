...
const TodayDealScreen = {
    ...
    render: async () => {
        ...
        const limitProducts = await products.filter(filterByCatogory);

        // Shuffling the items
        limitProducts.sort(() => Math.random() - 0.5);


        var getIdd = limitProducts.map(product => product._id);
        var name = limitProducts.map(product => product.name);



        var myUrl = `product/${getIdd}/${name}`;
...
<div class = "container " >
<h5 class = "fs-4 fw-bold mt-4 mb-4 w-75">Checkout Deals on All Products | 10% to 50% OFF </h5>
  <div class="row mt-3">
    <div class="col-md-10">
     <div class="row">
      ${(limitProducts)
    ...
    <ul class="navbar-nav me-auto" >
        <li class="nav-item" style = 'background-color: #192531;' >
            <a  class="nav-link text-light" style = "Margin-left: 10px;">
                <h5 class = "fs-5">10% to 50% OFF</h5>
            </a>
        </li>
        <li class="nav-item " style = "Margin-left: 10px;">
            <a href="/#/${categorydeal}/laptop" class="nav-link text-white">
              Laptop
            </a>
        </li>
        <li class="nav-item " style = "Margin-left: 10px;">
        ...
        <li class="nav-item ">
            <a href="/#/${categorydeal}/camera" class="nav-link text-light" style = "Margin-left: 10px;">
